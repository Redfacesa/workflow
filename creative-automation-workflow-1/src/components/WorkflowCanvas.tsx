...
import { 
  ...
  Settings, Trash2, Plus, GripVertical, ChevronDown, X, Check
} from 'lucide-react';
...
const WorkflowCanvas: React.FC<WorkflowCanvasProps> = ({
  ...
  const simulateRun = () => {
    ...
    // Find start nodes (no inputs connected)
    const connectedInputs = new Set(connections.map(c => c.toNodeId));
    const startNodes = nodes.filter(n => !connectedInputs.has(n.id));
    
    let delay = 0;
    const processQueue = [...startNodes.map(n => n.id)];
    ...
    const processNext = () => {
      ...
      setRunningNodes(prev => new Set([...prev, currentId]));
      ...
      setTimeout(() => {
        ...
        setCompletedNodes(prev => new Set([...prev, currentId]));
        
        // Find downstream nodes
        const downstream = connections
          .filter(c => c.fromNodeId === currentId)
          .map(c => c.toNodeId);
        processQueue.push(...downstream);
        processNext();
      }, 1500);
    };
    ...
    return (
      ...
      >
        <div className={`
          ...
          ${isComplete ? 'border-green-500/50 shadow-lg shadow-green-500/10' : ''}
    ...
    <div className="relative w-full h-full bg-[#0d0d1a] overflow-hidden rounded-xl border border-gray-800/50">
      ...
      <div className="absolute top-4 left-4 right-4 z-30 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={simulateRun}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-green-600 to-emerald-600 text-white font-medium hover:from-green-500 hover:to-emerald-500 transition-all text-sm shadow-lg shadow-green-500/20"
          >
